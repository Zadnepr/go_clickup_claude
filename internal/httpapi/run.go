...
//
// С непустым taskID условие триггера (тег/статус) не проверяется — только
// принадлежность настроенному списку и дедупликация (см.
// queue.RunOptions.SkipEligibility): явный запуск конкретной задачи — уже
// решение оператора, а не то же самое, что вебхук/сверка обнаружили её по
// тегу. Без taskID (массовое пересканирование) условие триггера обязательно
// как обычно — это то же самое, что делает сверка.
...
// newRunHandler обрабатывает POST /api/run — см. ManualRunner насчёт того,
// какие условия применяются к запуску конкретной задачи, а какие — только
// к массовому пересканированию.
